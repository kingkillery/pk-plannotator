...
| `PLANNOTATOR_GLIMPSE` | auto-detect | Set to `1` to force Glimpse native-window mode. Set to `0` to force browser mode. |
...
Set `PLANNOTATOR_GLIMPSE=1` to open Plannotator in a Glimpse native window when the optional dependency is installed. Use `--glimpse` or `--no-glimpse` for one-off overrides.
