...
| `PLANNOTATOR_GLIMPSE` | auto-detect | Set to `1` to force Glimpse native-window mode. Set to `0` to force browser mode. Also settable per-invocation with `--glimpse` / `--no-glimpse`. |
