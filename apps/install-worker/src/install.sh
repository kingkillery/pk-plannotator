...
mkdir -p "$INSTALL_DIR"
cache_dir="${XDG_CACHE_HOME:-$HOME/.cache}/pk-plannotator"
mkdir -p "$cache_dir"
...
) || echo "Warning: Glimpse dependency install failed; browser mode will still work."
...
rm -rf "${XDG_CACHE_HOME:-$HOME/.cache}/jiti" 2>/dev/null || true
    ...
    if [ -f "$GEMINI_SETTINGS" ]; then
        if ! grep -q '"plannotator"' "$GEMINI_SETTINGS" 2>/dev/null; then
            ...
            if command -v node &>/dev/null; then
                GEMINI_SETTINGS="$GEMINI_SETTINGS" PLANNOTATOR_HOOK="$PLANNOTATOR_HOOK" node <<'NODE_SCRIPT'
                  const fs = require('fs');
                  const settingsPath = process.env.GEMINI_SETTINGS;
                  const hook = JSON.parse(process.env.PLANNOTATOR_HOOK);
...
echo "  export PLANNOTATOR_GLIMPSE=1   # open Plannotator in a Glimpse native window when available"
